...
class Profil extends Component
{
    ...
    private function applyKtpData(KtpData $data)
    {
        ...
        if ($data->tempatLahir) {
            $this->tempat_lahir = $data->tempatLahir;
        }
        if ($data->tanggalLahir) {
            $this->tanggal_lahir = $data->tanggalLahir;
        }
        if ($data->jenisKelamin) {
            $this->jenis_kelamin = $data->jenisKelamin;
        }
        if ($data->agama) {
            $this->agama = $data->agama;
        }
        if ($data->statusKawin) {
            $this->status_perkawinan = $data->statusKawin;
        }
        if ($data->pekerjaan) {
        ...
        if ($data->golonganDarah) {
            $this->golongan_darah = $data->golonganDarah;
        }
        if ($data->alamat) {
            $this->alamat = $data->alamat;
        }
        if ($data->provinsi) {
            $this->propinsi = $data->provinsi;
        }
        if ($data->kabupaten) {
    ...
    {
        $riwayatPengurus = Pengurus::where('anggota_id', $this->anggota->id)
            ->orderBy('periode_mulai', 'desc')
            ->get();

        return view('livewire.anggota.profil', [
            'riwayatPengurus' => $riwayatPengurus,
        ])
